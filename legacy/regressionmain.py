...
def main():
    ...
    log_header = (logdataset.RegDataSet().get_log_header())
    ...
    while True:
        ...
        for ticker in top_volume_ticker:
            ...
            logging_data = logdataset.RegDataSet(
                nowTime=now_time,
                symbol=symbol,
                ...
                prevDayMaVolPerMaVol=prev_day_ma_vol_per_ma_vol,
            ...
            if reg_data_window.get(symbol, None) is None:
                reg_data_window[symbol] = []

            reg_data_window[symbol].append(logging_data)

            if len(reg_data_window[symbol]) >= reg_data_window_length:
                compare_data_pd_ap = reg_data_window[symbol][-reg_data_window_length].prev_day_macd_avg_per
                if compare_data_pd_ap != 0 and not np.isnan(compare_data_pd_ap):
                    compare_data = reg_data_window[symbol][-reg_data_window_length]
                    compare_data.price_dif = last_price / compare_data.last_price

                    compare_data.over_buy_limit = 1 if compare_data.price_dif > buy_limit else 0
                    compare_data.over_sell_limit = 1 if compare_data.price_dif < sell_limit else 0

                    bh.write_log(reg_log_path, compare_data)

                del reg_data_window[symbol][0]
