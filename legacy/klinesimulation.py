...
def get_candle(api: settings.ApiInfo, symbols: iter, limit: int):
    client = Client(api_key=api.key, api_secret=api.secret)
    candles = bh.get_kline(cl=client, symbols=symbols, limit=limit)
...
tickers = get_candle(api=settings.ApiInfo(), symbols=("ETHUSDT",), limit=1500)
...
futures_ticker = get_ticker(settings.ApiInfo())
...
for ticker in tickers[0]:
    ...
    if is_nan_ind == [False, False, False]:
        ...
        plot_dict["last_price"].append(last_price)
        plot_dict["ind_score"].append(ind_score_1)
...
ax1.plot(plot_dict["last_price"], color="red")
ax2 = ax1.twinx()
ax2.plot(plot_dict["ind_score"], color="blue")
